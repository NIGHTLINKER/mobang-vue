  ...
  <div>
    <div>
      ...
      <div v-if="lists.length" class="d-flex flex-wrap justify-content-center p-3">
        <div class="border-top shadow ml-3 mt-3 card-wrapper" v-for="list in lists" :key="list.id" @click="getDetail(list.id)">
...
export default {
  ...
  async mounted(){
    ...
    // Promise.all은 일괄적으로 요청을 보내서 리턴을 받기때문에 순서가 보장되지 않는다.
    // for of는 순서가 보장된다 (하지만 일괄요청이 아니기때문에 하나 처리하고 다음거 시작하는식임)
